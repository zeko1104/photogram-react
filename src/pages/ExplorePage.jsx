    ...
    <div className="min-h-screen flex flex-col items-center dark:bg-neutral-950 dark:text-white  p-4">
      ...
      <Box sx={{ width: 800, maxWidth: 1000, height: "auto", overflowY: "hidden" }}>
        <ImageList variant="masonry" cols={3} gap={8} className="w-full">
          {itemData.map((item) => (
            <ImageListItem key={item.img} className="rounded-lg shadow-md overflow-hidden">
              <img
              ...
              />
              <ImageListItemBar position="below" title={item.author} className="text-center text-sm font-medium" />
            </ImageListItem>
          ))}
        </ImageList>
      </Box>
